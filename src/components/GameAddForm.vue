          ...
          <n-form :model="gameForm" label-width="120" class="tab-form">
            ...
            <n-form-item :label="$t('gameForm.fields.genre')">
              <n-select ref="genreSelectRef" v-model:value="gameForm.genre" filterable multiple tag
                :options="genreOptions" :placeholder="$t('gameForm.placeholders.selectGenres')" :loading="loadingGenres"
            ...
            <n-form-item :label="$t('gameForm.fields.developer')">
              <n-select ref="developerSelectRef" v-model:value="gameForm.developer" filterable multiple tag
                :options="developerOptions" :placeholder="$t('gameForm.placeholders.selectDevelopers')"
            ...
            <n-form-item :label="$t('gameForm.fields.publisher')">
              <n-select ref="publisherSelectRef" v-model:value="gameForm.publisher" filterable multiple tag
                :options="publisherOptions" :placeholder="$t('gameForm.placeholders.selectPublishers')"
            ...
            <n-form-item :label="$t('gameForm.fields.tags')">
              <n-select ref="tagSelectRef" v-model:value="gameForm.tags" filterable multiple tag :options="tagOptions"
                :placeholder="$t('gameForm.placeholders.selectTags')" :loading="loadingTags" :render-tag="renderTag"
            ...
            <n-form-item :label="$t('gameForm.fields.monitorMode')">
              <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                ...
                <n-tooltip trigger="hover" placement="top">
                  <template #trigger>
                    <n-icon size="24" class="hint-icon">
                      <InfoOutlined />
                    </n-icon>
                  </template>
                  <div style="max-width: 280px; line-height: 1.5;">
                    {{ $t('gameForm.tooltip.monitorModeInfo') }}<br>
                    • <strong>{{ $t('gameForm.tooltip.fileMode') }}</strong> {{ $t('gameForm.tooltip.fileModeDesc')
                    }}<br>
                    • <strong>{{ $t('gameForm.tooltip.folderMode') }}</strong> {{ $t('gameForm.tooltip.folderModeDesc')
                    }}<br>
                    • <strong>{{ $t('gameForm.tooltip.processMode') }}</strong> {{
                      $t('gameForm.tooltip.processModeDesc') }}
                    <hr>
                    <strong>{{ $t('gameForm.tooltip.attention') }}</strong>
                  </div>
                </n-tooltip>
              </div>
            </n-form-item>
            ...
            <n-form-item v-if="gameForm.procMonMode === PROC_MON_MODE.PROCESS"
              ...
              <div class="process-names-container">
                <div class="process-names-header">
                  <span class="hint-text">{{ $t('gameForm.empty.processNamesHint') }}</span>
                ...
                <div v-else class="no-process-names">
                  <p class="hint-text">{{ $t('gameForm.empty.noProcessNames') }}
                  </p>
                </div>
              </div>
            </n-form-item>
            ...
            <div class="links-container">
              ...
              <div v-if="gameForm.links && gameForm.links.length > 0" class="links-list">
                <div v-for="(link, index) in gameForm.links" :key="index" class="link-item">
                  ...
                  <n-form-item label="" :path="`links.${index}.name`">
                    <n-input v-model:value="link.name" :placeholder="$t('gameForm.placeholders.linkName')" />
                  </n-form-item>

                  <n-form-item label="" :path="`links.${index}.url`">
                    <n-input v-model:value="link.url" :placeholder="$t('gameForm.placeholders.url')" />
                  </n-form-item>
                </div>
              </div>
            ...
            <div class="actions-container">
              ...
              <div v-if="gameForm.actions && gameForm.actions.length > 0" class="actions-list">
                <div v-for="(action, index) in gameForm.actions" :key="index" class="action-item">
                  ...
                  <n-form-item :label="$t('gameForm.fields.type')">
...
<script setup lang="ts" name="GameAddForm">
  ...
  async function saveGame() {
    ...
    try {
      ...
      message.error(t('gameForm.messages.addFailed'))
    } finally {
      saving.value = false
    }
  }
...
<style src="@/styles/game-form.css"></style>
